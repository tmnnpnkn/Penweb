                ...
                <div class="row">
                    <div class="col-md-10">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php
                                $sql_order_details = "SELECT d.*, p.*, t.type_name, p.p_detail
                            FROM order_details d
                            INNER JOIN product p ON d.p_id = p.p_id
                            INNER JOIN tbl_product_type t ON p.type_id = t.type_id
                            WHERE d.orderID='$invoiceId'";
                        ...
                        <h6>รวมเป็นเงิน (ไม่รวม VAT):
                            <?= number_format($totalPriceWithoutVAT, 2) ?> บาท
                        </h6>
                        <h6>VAT (7%):
                            <?= number_format($vat, 2) ?> บาท
                        </h6>
                        <h6>รวมเป็นเงิน (รวม VAT):
                            <?= number_format($totalPriceWithVAT, 2) ?> บาท
                        </h6>

                        <a href="confirm_order.php" class="btn btn-back btn-primary">กลับ</a>
                    </div>
                </div>
            </div>
        </body>
